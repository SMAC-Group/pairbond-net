    ...
    g <- reactive({
       ...
       makeTent = function(ints) {
         ...
         if((length(ints) %% 2) == 0) {
           ...
           ints_tent = c(rev(ints_o[seq.int(from = 1, to = (length(ints) - 1), by = 2)]),
                         (ints_o[seq.int(from = 2, to = length(ints), by = 2)]))
         } else {
           # odd number of observations
           ints_tent = c(rev(ints_o[seq.int(from = 2, to = (length(ints) - 1), by = 2)]),
                         ints_o[seq.int(from = 1, to = length(ints), by = 2)])
         }
         return(ints_tent)
       }
